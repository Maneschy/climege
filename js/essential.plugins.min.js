/*
	.THIS FILE CONTAINS ESSENTIAL JQUERY PLUGINS FOR GEARBOX HTML TEMPLATE
	.TEMPLATE/JS/ALL DIRECTORY CONTAINS ALL  JQUERY PLUGINS USED BY GEARBOX.
 ...
 */
 ...
 //NOTE : Twitterjs is not a JQuery plugin
 ...
 * Dual licensed under the MIT and GPL licenses:
	...
	NOTE : we've customized this plugin. - you'll get our own Lightbox script in the next versions of the template.
 */
...
$('body').append(tmp=$('<div id="fancybox-tmp"></div>'),loading=$('<div id="fancybox-loading"><div></div></div>'),overlay=$('<div id="fancybox-overlay"></div>'),wrap=$('<div id="fancybox-wrap"></div>'));outer=$('<div id="fancybox-outer"></div>').appendTo(wrap);outer.append(content=$('<div id="fancybox-content"></div>'),close=$('<a id="fancybox-close" class="demention-close-icon">&#215</a>'),title=$('<div id="fancybox-title"></div>'),nav_left=$('<a href="javascript:;" id="fancybox-left"><span class="fancy-ico" id="fancybox-left-ico"></span></a>'),nav_right=$('<a href="javascript:;" id="fancybox-right"><span class="fancy-ico" id="fancybox-right-ico"></span></a>'));close.click($.fancybox.close);loading.click($.fancybox.cancel);nav_left.click(function(e){e.preventDefault();$.fancybox.prev();});nav_right.click(function(e){e.preventDefault();$.fancybox.next();});if($.fn.mousewheel){wrap.bind('mousewheel.fb',function(e,delta){if(busy){e.preventDefault();}else if($(e.target).get(0).clientHeight==0||$(e.target).get(0).scrollHeight===$(e.target).get(0).clientHeight){e.preventDefault();$.fancybox[delta>0?'prev':'next']();}});}
...
$(images[i]).css("visibility","visible").delay(delaySum).animate({opacity:1},options.fadein,function(){$(this).parent().addClass("loaded");});}}},options.check_timer)};images.each(function(){checkFlag[i++]=false;});images=$.makeArray(images);var icon=jQuery("<img />",{id:'loadingicon',src:'images/base/spinner.gif'}).hide().appendTo("body");timer=setInterval(function(){if(icon[0].complete==true)
...
//CUSTOM CODE -- custom.js contains the rest


/*the bellow conditions can detect ie lt 9 browsers and other browsers that we want to categorize as "old browsers"*/
if(!Modernizr.rgba || !Modernizr.opacity){
	$('html').addClass('old-browser'); // add .old-browser class to <html> element
	var oldBrowser = true ;
}		

//detect your UA to use later
var yourSide =  navigator.userAgent.toLowerCase();	